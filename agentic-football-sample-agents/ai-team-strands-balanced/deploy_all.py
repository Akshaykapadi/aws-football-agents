...
for tool in ["agentcore", "aws", "node", "npm", "uv"]:
    check_tool(tool)
...
try:
    ...
    # cdk.json launches dist/bin/cdk.js, so compile the generated TypeScript
    # application before either bootstrap or deploy tries to load it.
    print("==========================================")
    print("  Building CDK application")
    print("==========================================")
    run(["npm", "run", "build"], cwd=cdk_dir)
    print()
    ...
    print("  CDK Bootstrap")
    print("==========================================")
    # Use the project-pinned CDK CLI installed from package-lock.json. This
    # keeps deployment reproducible and avoids requiring a global CDK install.
    cdk_cli = cdk_dir / "node_modules" / "aws-cdk" / "bin" / "cdk"
    run(["node", str(cdk_cli), "bootstrap", f"aws://{account_id}/{region}"], cwd=cdk_dir)
    print()
...
print("  Memory:   team_memory (EPISODIC; ID injected as MEMORY_TEAM_MEMORY_ID)")
print()
