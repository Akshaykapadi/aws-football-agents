...
$ErrorActionPreference = "Continue"
Write-Host "  Running: agentcore create --name ai_gk_agent_ak --project-name $projectName --output-dir $buildDirName"
& $agentcorePath create --name "ai_gk_agent_ak" --project-name $projectName --build CodeZip --framework Strands --model-provider Bedrock --memory none --skip-git --skip-python-setup --output-dir $buildDirName 2>&1 | ForEach-Object { Write-Host "    $_" }
...
foreach ($agent in $allAgents) {
    $agentNameClean = ($agent -replace '-', '_') + "_agent_ak"
