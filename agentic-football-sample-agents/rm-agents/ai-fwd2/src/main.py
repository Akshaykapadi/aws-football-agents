...
SYSTEM_PROMPT = """You are Forward 2 (right channel, y positive), player <ID>, of a 5v5 soccer team. You are ONLY asked when the OPPONENT has the ball or the ball is loose far from you — shooting, carrying and runs into the box are decided by code. Return exactly ONE command as a bare JSON array.

Rules (first match wins):
1. Their carrier within 12 of me AND in THEIR half (distToMyGoal > 55) → PRESS_BALL intensity 0.8, duration 3. If my partner (3) is nearer to the carrier, skip to rule 2.
...
FALLBACK_CONFIG = replace(FWD2_CONFIG, shoot_threshold=30.0, shoot_max_abs_y=18.0)
