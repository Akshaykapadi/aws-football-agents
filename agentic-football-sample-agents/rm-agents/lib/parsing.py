...
# Every documented parameter name. Small models sometimes drop the "parameters"
# wrapper and put these at the top level of the command — we lift them back.
PARAM_KEYS = {
    "target_x", "target_y", "sprint", "target_player_id", "type", "aim_location",
    "power", "distance", "method", "intensity", "tightness", "aggressive",
    "target_team", "stance",
}
    ...
    for cmd in commands:
        ...
        if "commandType" in cmd:
            ...
            if cmd["commandType"] not in VALID_COMMANDS:
                continue
            # Lift flattened parameters into the "parameters" wrapper
            if not isinstance(cmd.get("parameters"), dict):
                cmd["parameters"] = {}
            for key in [k for k in cmd if k in PARAM_KEYS]:
                cmd["parameters"].setdefault(key, cmd.pop(key))
            ...
            # Fill parameters a small model tends to drop
            if cmd["commandType"] == "SHOOT":
                cmd["parameters"].setdefault("aim_location", "BR")
                cmd["parameters"].setdefault("power", 0.9)
            elif cmd["commandType"] == "PASS":
                cmd["parameters"].setdefault("type", "GROUND")
            elif cmd["commandType"] in ("PRESS_BALL", "MARK", "INTERCEPT", "FOLLOW_PLAYER"):
                if not cmd.get("duration"):
                    cmd["duration"] = 3
