...
def setup_lib_path(caller_file: str) -> None:
    ...
    Works for both layouts:
      ...
      - Local dev: rm-agents/lib/ is two levels up from agent/src/main.py
    """
    here = os.path.dirname(os.path.abspath(caller_file))
    lib_deployed = os.path.join(here, "..", "lib")              # agent/lib  (AgentCore, injected by deploy_all.py)
    lib_local = os.path.join(here, "..", "..", "lib")           # rm-agents/lib (local dev; this team is self-contained)
